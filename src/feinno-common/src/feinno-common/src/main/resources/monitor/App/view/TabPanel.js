				...
				Ext.Ajax.request({
					...
					success : function(resp, option) {// ajax请求发送成功时执行
						...
						for (var i = 0; i < body.reports.length; i++) {
							if (body.reports[i].report == null
									|| body.reports[i].report == undefined) {
								continue;
							}
							var category = body.reports[i].report.category;
							var time = body.reports[i].report.time;
							document.getElementById('time').innerHTML = "<center/>"
									+ "服务器端时间：" + time;
							var haveThisGrid = false;
							tab.items.each(function(item) {
								if (item.title == category) {
									var report = body.reports[i].report;
									haveThisGrid = true;
									var readData = [];
