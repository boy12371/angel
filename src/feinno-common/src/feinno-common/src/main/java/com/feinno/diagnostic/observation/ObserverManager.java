 ...
 * {@link ObserverManager#addInspector(Observable, ObserverReportMode, TimeSpan, com.feinno.diagnostic.observation.ObserverInspector.ReportCallback)}
 * 的ReportCallback中
 * <p>
...
//	private static final Logger LOGGER = LoggerFactory.getLogger(ObserverManager.class);
	...
	public static void addInspector(Observable observer, ObserverReportMode mode, TimeSpan span, ObserverInspector.ReportCallback callback)
	{
	...
	{
		Logger LOGGER = LoggerFactory.getLogger(ObserverManager.class);
		while (true) {
			try {
				// TODO 保证在每一秒的前沿时刻运行
				Thread.sleep(10);
