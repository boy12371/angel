...
public abstract class WrapsFieldInterpreter extends AbstractFieldInterpreter {
	 ...
	 * 的输出
	 */
	private static final String SIZE_CODE_TEMPLATE = "if( ${data}.${getterName}() != null) size += ${package_core}.CodedOutputStream.compute${streamType}Size(${number},${data}.${getterName}());";
	...
	public synchronized static FieldInterpreter getOptimizationByteFieldInterpreter() {
		if (byteOptimizationFieldInterpreter == null) {
			byteOptimizationFieldInterpreter = new WrapsFieldInterpreter(ProtoFieldType.BYTE_OBJECT) {
				 ...
				 */
				private static final String WRITE_CODE_ARRAY_TEMPLATE = "${output}.writeBytes(${number}, ${package_core}.ByteString.copyFrom(${package_core}.util.ArrayUtil.wrapsToPrimitive(${data}.${getterName}())));break;";

				private static final String SIZE_CODE_ARRAY_TEMPLATE = "size += ${package_core}.CodedOutputStream.computeBytesSize(${number},${package_core}.ByteString.copyFrom(${package_core}.util.ArrayUtil.wrapsToPrimitive(${data}.${getterName}())));break;";

				private static final String PARSE_CODE_ARRAY_TEMPLATE = "fieldValue = null; ${data}.${setterName}(${package_core}.util.ArrayUtil.primitiveToWraps(${input}.readBytes().toByteArray()));";
