...
//	}
	...
	RpcServerContext(RpcServerTransaction tx)
	{
		this.tx = tx;
	...
	{
		if (respForExts == null) {
			respForExts = RpcResponse.createResults(null);
		}		
		respForExts.putExtension(id, ext);
	}
	...
	private <R> void internalEnd(RpcReturnCode code, R results, Throwable error)
	{
		if (done.compareAndSet(false, true)) {
			...
			if (code == RpcReturnCode.OK) {
				response = RpcResponse.createResults(results);
			} else {
				response = RpcResponse.createError(code, error);
			}
			if (respForExts != null) {
