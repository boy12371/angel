...
//	private IntegerDictionary<RpcClientMethodCache> clientCaches;
	...
	{
		LOGGER.info("auto connect to {}", this.getRemoteEndpoint());
		setStatus(RpcConnectionStatus.CONNECTING);
		Future<Throwable> future = connect();

		future.addListener(new EventHandler<Throwable>() {
			@Override
			public void run(Object sender, Throwable args)
			{
				LOGGER.info("connect future {}", args);
				if (args == null) {
					...
					while (request != null) {
						try {
							...
							tx.setResponse(RpcResponse.createError(RpcReturnCode.CONNECTION_FAILED, ex));
						}
					...
					while (request != null) {
						int seq = request.getHeader().getSequence();
						RpcClientTransaction tx = RpcClientTransactionManager.INSTANCE.removeTransaction(seq);
						tx.setResponse(RpcResponse.createError(RpcReturnCode.CONNECTION_FAILED, args));
						request = pendings.poll();
					}
				}
			}
	...
	public final void sendRequest(RpcClientTransaction tx, RpcRequest request)
	{
		...
		if (seq < 0) {
			tx.setResponse(RpcResponse.createError(RpcReturnCode.TRANSACTION_TO_MANY, null));
			return;
		}
		...
		try {
			...
			tx.setResponse(RpcResponse.createError(RpcReturnCode.SEND_FAILED, ex));
		}
	}
