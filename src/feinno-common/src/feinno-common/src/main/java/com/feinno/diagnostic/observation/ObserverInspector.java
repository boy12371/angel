		...
		if (lastItems == null) {
			...
			case SUMMARY:
				...
				for (Item i: lastItems) {
					...
					ObserverReportUnit u = s.computeReport(i.snapshot);
					units.add(u);
				}
				if (units.size() > 0) {
			...
			case ALL:
				for (Item i: lastItems) {
					ObservableUnit unit = i.unit;
					ObserverReportSnapshot s = unit.getObserverSnapshot();
					ObserverReportUnit u = s.computeReport(i.snapshot);
					u.outputReport(report.newRow(unit.getInstanceName()));
				}
			}
		}
