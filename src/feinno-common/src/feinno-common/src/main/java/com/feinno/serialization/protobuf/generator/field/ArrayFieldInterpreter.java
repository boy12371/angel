...
public class ArrayFieldInterpreter extends AbstractFieldInterpreter {
	 ...
	 * 		com.feinno.serialization.protobuf.util.ArrayUtil.listToArray(arrayTypetMap.get(513),array);
	 * 		data.setBoolean_Object_Array(array);
	 * 	}
	 * </code>
	 */
	private static final String CONVER_ARRAY_CODE_TEMPLATE = "if(arrayTypetMap.get(${number}) != null && arrayTypetMap.get(${number}).size() > 0){ ${java_type}[] array = new ${java_type}[arrayTypetMap.get(${number}).size()];com.feinno.serialization.protobuf.util.ArrayUtil.listToArray(arrayTypetMap.get(${number}),array); ${data}.${setterName}(array);}";
