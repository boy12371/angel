package com.feinno.serialization.protobuf.generator.field;
...
public class MapFieldInterpreter extends AbstractFieldInterpreter {
	 ...
	 * 			new com.feinno.serialization.protobuf.util.Action(){
	 * 				public void run(){
	 ...
	 * 的输出
	 */
	private static final String SIZE_CODE_TEMPLATE = "if( ${data}.${getterName}() != null) {  for (final java.util.Map.Entry<${java_type1}, ${java_type2}> entry :  ${data}.${getterName}().entrySet()) {int sizeTemp = 0; sizeTemp+=new ${package_core}.util.Action<Integer>(){ public Integer run(){ int size = 0 ;${java_type1} value = entry.getKey();  ${key_size} return size;} }.run(); sizeTemp+=new ${package_core}.util.Action<Integer>(){ public Integer run(){ int size = 0 ;${java_type2} value = entry.getValue();  ${value_size} return size;} }.run(); size += com.feinno.serialization.protobuf.CodedOutputStream.computeTagSize(${number}); size+=${package_core}.CodedOutputStream.computeRawVarint32Size(sizeTemp); size += sizeTemp; } }";

	/**
	 * 表示反序列化代码的格式，期待一个类似于<br>
	 * <code>
	 * //第一步从MAP重找出此序号对应的集合
	 * java.util.List<com.feinno.serialization.protobuf.util.TwoTuple<${java_type1},${java_type2}>> listTemp = (java.util.List<com.feinno.serialization.protobuf.util.TwoTuple<${java_type1},${java_type2}>>)arrayTypetMap.get(${number});
	 * if (listTemp == null){//如果这个序号对应的集合不存在，则创建这个集合,并且向这个集合中加入一个空的二元组，每个二元组代表一对Key Value
	 * 		listTemp = new java.util.LinkedList<com.feinno.serialization.protobuf.util.TwoTuple<${java_type1},${java_type2}>>();
	 ...
	 * 			case 0:
	 * 				isRun = false;
	 * 				com.feinno.serialization.protobuf.util.TwoTuple<${java_type1},${java_type2}> entry = listTemp.get(listTemp.size()-1);
	 ...
	 * 				${parse_array1}
	 * 				com.feinno.serialization.protobuf.util.TwoTuple<${java_type1},${java_type2}> entryTemp = listTemp.get(listTemp.size()-1);
	 * 				if(entryTemp == null){
	 * 					entryTemp = new com.feinno.serialization.protobuf.util.TwoTuple<${java_type1},${java_type2}>(null,null);
