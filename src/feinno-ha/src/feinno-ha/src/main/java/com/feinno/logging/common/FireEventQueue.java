package com.feinno.logging.common;
		...
		if (fireEvent != null) {
			Thread thread = new Thread() {
				...
				public void run() {
					...
					while (isRun) {
						Set<E> expiredDatas = new HashSet<E>();
						synchronized (set) {
							...
							while (iterable.hasNext()) {
								...
								if (elementsDecoration.isTime()) {
									iterable.remove();
									expiredDatas.add(elementsDecoration.getData());
								}
							}
						}
						for (E e : expiredDatas) {
							try {
								fireEvent.run(e);
							} catch (Exception ex) {// 防止回调中出现异常导致线程终止
								LOGGER.error(
